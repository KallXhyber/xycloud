...
// --- Elemen & Fungsi UI Dasar ---
const toastContainer = document.getElementById('toast-container');
const successSound = document.getElementById('success-sound');
const errorSound = document.getElementById('error-sound');
successSound.volume = 0.5; errorSound.volume = 0.5;
...
function showToast(message, type = 'success') { 
    ...
    if (type === 'success') successSound.play().catch(e => {}); else errorSound.play().catch(e => {}); 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', async (e) => { 
        ...
        if (target.classList.contains('set-waktu-btn')) { 
            const pcId = e.target.dataset.id; 
            const container = e.target.closest('.backdrop-blur-custom'); 
            ...
            try { await updateDoc(doc(db, 'pcs', pcId), { endTime: endTime }); showToast(`Timer untuk ${minutes} menit berhasil diatur!`, 'success'); input.value = ''; } catch (err) { showToast('Gagal mengatur timer.', 'error'); } 
        } 
    });
    ...
    onSnapshot(collection(db, `pcs`), (snapshot) => { 
        ...
        countdownIntervals = {}; 
        if (snapshot.empty) { pcMonitoringDiv.innerHTML = '<p class="col-span-full text-center text-gray-400 backdrop-blur-custom p-4 rounded-lg">Belum ada PC yang dikonfigurasi.</p>'; return; } 
